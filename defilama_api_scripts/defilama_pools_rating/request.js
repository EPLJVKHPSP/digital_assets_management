...
async function updateRatings() {
    ...
    parse(inputData, { columns: true, trim: true, skip_empty_lines: true }, async (err, pools) => {
        if (err) {
        ...
        // Sort pools by Rating in descending order
        updatedPools.sort((a, b) => {
            return parseFloat(b.Rating.replace(/[^0-9.-]+/g, "")) - parseFloat(a.Rating.replace(/[^0-9.-]+/g, ""));
        });
        ...
        const csvWriter = createCsvWriter({
            ...
            header: [
                ...
                { id: 'ROI', title: 'ROI' } // Update CSV header to include new columns
            ]
        });

        csvWriter.writeRecords(updatedPools)
            .then(() => {
